...
my $scf="";
my $seq="";
my $ext_length=200;#this controls how much of an extension for the transcripts we allow for protein alignments that go beyond the transcript boundaries
...
while(my $line=<FILE>){
  ...
  if($gtf_fields[2] eq "transcript"){
    ...
    if($class_code =~ /k|=|u|i|y|o|x/ || ($class_code =~ /n|j|m/  && ($protein_start{$protID} > $tstart-$ext_length  && $protein_end{$protID} < $tend+$ext_length))){
