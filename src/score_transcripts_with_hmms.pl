  ...
  for(my $j=1;$j<=$#{$transcript_gff{$g}};$j++){
    ...
    if($j>0 && defined($ARGV[2])){
      ...
      if($use_negatives == 0){
        ...
        $junction0_score=($donor_score-$donor_nscore+$acceptor_score-$acceptor_nscore)+2.5;
        $junction1_score=($donor_hmm_score-$donor_hmm_nscore+$acceptor_hmm_score-$acceptor_hmm_nscore)*0.45+2.5;
        $junction2_score=($donor_hmm2_score-$donor_hmm2_nscore+$acceptor_hmm2_score-$acceptor_hmm2_nscore)*.32+2.5;
      }
