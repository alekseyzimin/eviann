...
if [ ! -e transcripts_assemble.success ];then
  if [ -s $ALT_EST ];then
    ...
    if [ ! -s tissue0.bam.sorted.bam.gtf ];then 
      minimap2 --split-prefix -a -u f -x splice:hq -t $NUM_THREADS -G $MAX_INTRON $GENOMEFILE $ALT_EST 2>tissue0.err | \
