...
function usage {
 ...
 echo " --min-prot            minimum protein length (in amino-acids) for ab initio ORF detection without homology evidence, default: 100"
...
# the file $GENOME.palign.all.gff contains all CDSs we need to use
  ...
  TransDecoder.LongOrfs -m $MIN_ORF -S -t $GENOME.broken_cds.fa -m 75 1>transdecoder.LongOrfs.out 2>&1 && \
