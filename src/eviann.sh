...
if [ ! -e align.success ];then
  ...
  if [ -s $ALT_EST ];then
    echo "if [ ! -s tissue0.bam ];then minimap2 -a -x splice:hq -t $NUM_THREADS -G $MAX_INTRON $GENOMEFILE $ALT_EST 2>tissue0.err | samtools view -bhS /dev/stdin > tissue0.bam.tmp && mv tissue0.bam.tmp tissue0.bam; fi;" >> hisat2.sh
  fi
